...
export default function SinistresPage() {
  ...
  const eligibleDevices =
    devices?.filter((d) => {
      ...
      return subscriptions?.some(
        (s) => s.device_id === d.id && s.status === "active",
      );
    }) ?? [];
  ...
  const handleSubmit = async () => {
    ...
    const sub = subscriptions?.find(
      (s) => s.device_id === selectedDeviceId && s.status === "active",
    );
    if (!sub) return;
    ...
    try {
      ...
      logger.error("claims: create claim failed", { error: err });
    }
  };
      ...
      <div className="mx-auto max-w-3xl px-5 md:px-8">
        ...
        {createClaim.isError && (
          <div className="mb-6 rounded-2xl border border-red-200/80 bg-red-50 px-4 py-3 text-sm font-medium text-red-600">
            {(createClaim.error as { message?: string })?.message ||
              (lang === "fr"
                ? "Impossible de soumettre le sinistre."
        ...
        {tab === "new" && (
          <div className="rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-sm md:p-8">
            <h2 className="mb-6 text-lg font-medium text-indigo-950">
              {t.claims.new}
            </h2>
            <div className="space-y-5">
              <FormField label={t.claims.device}>
                <Select
                  ...
                  onChange={(e) =>
                    setSelectedDeviceId((e.target as HTMLSelectElement).value)
                  }
                >
                  <option value="">
                    {lang === "fr"
              ...
              <FormField label={t.claims.type}>
                <div className="grid grid-cols-2 gap-3">
                  {(Object.entries(t.claims.types) as [string, string][]).map(
                    ([key, label]) => {
