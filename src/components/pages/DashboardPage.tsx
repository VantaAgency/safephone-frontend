...
export default function DashboardPage() {
  ...
  const handleClaimSubmit = async () => {
    ...
    try {
      ...
      logger.error("dashboard: create claim failed", { error: err });
    }
  };
  ...
  const handleResumeCheckout = async (paymentId: string) => {
    ...
    try {
      ...
      logger.error("dashboard: resume payment failed", { error: err });
      setPaymentActionError(
        lang === "fr"
  ...
  const handleRenewSubscription = async (subscription: Subscription) => {
    ...
    try {
      ...
      logger.error("dashboard: renew subscription failed", { error: err });
      setRenewalActionError(
        lang === "fr"
        ...
        {tab === "repairs" && (
          <div>
            <div className="mb-6">
              <h2 className="text-xl font-medium text-indigo-950">
                {lang === "fr"
                  ? `Mes réparations ${repairBrand}`
                  : `My ${repairBrand} repairs`}
              </h2>
            ...
            {repairsLoading ? (
              ...
              <EmptyState
                ...
                description={
                  lang === "fr"
                    ? `Vos demandes ${repairBrand} apparaîtront ici après soumission.`
                    : `Your ${repairBrand} requests will appear here after submission.`
                }
                action={{
                  label:
