...
import type { CheckoutResult, PaymentStatus } from "@/lib/api/types";
...
function formatTrackedDeviceName(
  ...
  const brand = checkout?.device?.brand?.trim() ?? "";
  const model = checkout?.device?.model?.trim() ?? "";
...
function trackedStatusCopy(status: PaymentStatus | undefined, lang: "fr" | "en") {
  switch (status) {
    case "completed":
    case "refunded":
      return {
        title:
          lang === "fr" ? "Paiement deja effectue" : "Payment already completed",
        subtitle:
          lang === "fr"
    ...
    case "expired":
      return {
        title:
          lang === "fr" ? "Paiement a relancer" : "Restart your payment",
        subtitle:
          lang === "fr"
    ...
    default:
      return {
        title:
          lang === "fr" ? "Suivi de votre paiement" : "Track your payment",
        subtitle:
          lang === "fr"
...
function PaiementContent() {
  ...
  const currentCreateQueryKey = useMemo(
    () => `${planId}:${brandSlug}:${annual ? "annual" : "monthly"}`,
    [annual, brandSlug, planId],
  );
  ...
  const fullDeviceName = useMemo(() => {
    if (!selectedBrandSlug && !trimmedModel) {
      return lang === "fr"
        ? "Votre telephone sera confirme ici"
        : "Your phone will be confirmed here";
    }

    if (!trimmedModel) {
      return previewBrandLabel
        ? `${previewBrandLabel} ${lang === "fr" ? "— modele a confirmer" : "— model to confirm"}`
        : lang === "fr"
          ? "Modele a confirmer"
          : "Model to confirm";
    }
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (result.payment?.id) {
        router.replace(`/paiement?payment_id=${result.payment.id}&redirect=checkout`);
      }
    } catch (err) {
        ...
        <div className="mb-6 rounded-[2rem] border border-yellow-300/40 bg-yellow-50 p-5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between text-sm text-slate-500">
              ...
              <span>{summaryPlanName ? `SafePhone ${summaryPlanName}` : "SafePhone"}</span>
            </div>
          </div>
